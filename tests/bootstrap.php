...
namespace {
    ...
    /**
     * Core Configuration's scope resolution (1.7.6.0 classes/Configuration.php): get() resolves the shop and group
     * ids as core does (L217-231: the context's, null outside multistore) then cascades shop -> group -> global, else
     * FALSE; hasKey() reads one bucket (L347-353); updateValue() writes one row at the resolved scope and skips a value
     * that already cascades to the same thing (L451-455); deleteByName() drops every row (L552-577); deleteFromContext()
     * drops the context's row and no-ops in all-shops (L584-609). get()'s 2nd parameter is honoured as $default here;
     * core's is $id_lang and an absent row answers FALSE, so a module call site relying on a slot-two default passes
     * here and reads false in production - that default is fiction, not a modelled behaviour.
     */
    class Configuration
    {
        ...
        public static function deleteFromContext($key): void
        {
            ...
            $idShopGroup = Shop::getContextShopGroupID(true);
            $idShop = Shop::getContext() == Shop::CONTEXT_SHOP ? Shop::getContextShopID(true) : null;
            if ($idShop) {
                unset(StubStore::$configurationShop[$idShop][$key]);
            } elseif ($idShopGroup) {
                unset(StubStore::$configurationGroup[$idShopGroup][$key]);
    ...
    /** Stands in for the controller's die() so a spec can assert on the response. */
    ...
    /** Core's shop context (1.7.6.0 classes/shop/Shop.php L108-110, L925-936, L962-1051, L1159-1169). */
    class Shop
    {
